...
const portfolioCases = [
  {
    ...
    image: "/cases/case-1.jpg" // placeholders
  },
  {
    id: "02",
    client: "AURA BEAUTY",
...
export default function Cases() {
  ...
  // Moves the horizontal container to the left as vertical scroll progresses
      ...
      <div className="sticky top-0 flex h-screen items-center overflow-hidden">
        ...
        <motion.div style={{ x }} className="flex gap-12 md:gap-24 px-4 md:px-12 pl-[5vw] md:pl-[20vw] pt-24">
          {portfolioCases.map((projeto) => (
            <div 
              key={projeto.id} 
              className="group relative h-[60vh] md:h-[70vh] w-[85vw] md:w-[60vw] flex-shrink-0 cursor-pointer overflow-hidden rounded-sm bg-[#0a0a0a] border border-white/10 p-6 flex flex-col justify-between"
            >
              {/* Image background placeholder */ }
              <div className="absolute inset-0 z-0 overflow-hidden bg-[#111]">
                 {/* Aqui entrará a tag <Image /> futuramente */}
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent z-10"></div>
                <div className="absolute inset-0 bg-[#151515] group-hover:scale-105 transition-transform duration-700 ease-out z-0"></div>
              </div>
              ...
              <div className="relative z-20 flex items-end justify-between">
                <h4 className="text-3xl md:text-5xl xl:text-6xl font-light tracking-tight max-w-[80%] leading-[1.1]">
                  {projeto.title}
                </h4>
                <div className="font-serif italic text-6xl md:text-8xl lg:text-9xl text-white/10 font-bold leading-none -mb-4 tracking-tighter">
                  {projeto.id}
                </div>
              </div>
            </div>
